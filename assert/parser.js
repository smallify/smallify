...
smallify.register(
  async function (ins1) {
    ...
    // assert(ins1.hasContentTypeParser('parser/ins2'), 'ins1 not parser/ins2')
  },
  {
    name: 'ins1'
  }
...
smallify.register(
  async function (ins2) {
    ...
    assert(ins2.hasContentTypeParser('text/plain'), 'ins2 not text/plain')

    ins2.addContentTypeParser('parser/ins2', async function (req) {})

    assert(ins2.hasContentTypeParser('parser/ins2'), 'ins2 not parser/ins2')
    assert(ins2.hasContentTypeParser('parser/ins1'), 'ins2 not parser/ins1')
  },
  {
    name: 'ins2'
  }
